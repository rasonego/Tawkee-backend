...
export class ScheduleValidationService {
  ...
  isBeforeMaxAdvance(date: Date, schedule: ScheduleSettingsDto): boolean {
    ...
    const now = new Date();
    return date.getTime() - now.getTime() <= schedule.maxAdvanceDays * 24 * 60 * 60 * 1000;
  }
  ...
  isDurationAllowed(start: Date, end: Date, schedule: ScheduleSettingsDto): boolean {
  ...
  validateSchedule(
    ...
    if (!this.isWithinAvailableHours(start, schedule) || !this.isWithinAvailableHours(end, schedule)) {
  ...
  async updateScheduleSettings(agentId: string, updateDto: Partial<ScheduleSettingsDto>) {
    ...
    if (!existing) {
      throw new NotFoundException(`Schedule settings for agent ${agentId} not found`);
    }
